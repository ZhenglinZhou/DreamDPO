...
class RandomMultiviewCameraDataModuleConfig(RandomCameraDataModuleConfig):
    ...
    front_view_aug: bool = False
    side_view_aug: bool = False
...
class RandomMultiviewCameraIterableDataset(RandomCameraIterableDataset):
    ...
    def collate(self, batch) -> Dict[str, Any]:
        ...
        if self.cfg.front_view_aug and random_value < 0.5:
            azimuth_deg = torch.zeros_like(elevation_deg)
        elif self.cfg.side_view_aug and random_value < 0.5:
            deg = 90 if random_value < 0.25 else -90
            azimuth_deg = torch.ones_like(elevation_deg) * deg
        else:
            azimuth_deg = (
        ...
        if self.cfg.light_sample_strategy == "dreamfusion":
            ...
            light_direction: Float[Tensor, "B 3"] = F.normalize(
                camera_positions
                + torch.randn(real_batch_size, 3).repeat_interleave(self.cfg.n_view,
                                                                    dim=0) * self.cfg.light_position_perturb,
                dim=-1,
            )
